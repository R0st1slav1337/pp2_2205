...
class DrawingScene(SceneBase):
    def __init__(self):
        ...
        self.mode = 'brush' # 'brush', 'circle', 'square', 'eraser'
    ...
    def ProcessInput(self, events, pressed_keys):
        for event in events:
            if event.type == pygame.MOUSEBUTTONDOWN: # Check if the mouse button is pressed
                ...
                if self.start_pos and self.mode in ['circle', 'square']: # Draw the shape if the mode is circle or square
    ...
    def draw_shape(self, end_pos):
        ...
        if self.mode == 'circle':
            ...
            pygame.draw.rect(screen, self.brush_color, rect) # Draw the square
